...
test("host selectors resolve live branches and ID prefixes, not paths or revisions; ambiguity is refused", async (t) => {
  ...
  for (const selector of ["main", id, id.slice(0, 8)])
    assert.equal(await resolveSelection(config, selector), id);
  ...
  const options = {
    ...
    service: {
      ...
      launch: async (path: string) => {
        launches++;
        lock = await acquireWorktreeLock(f.config.runtimeRoot, path);
      },
    },
  };
