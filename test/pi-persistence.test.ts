...
const runPi = async (agentDir: string, code: string, args: string[] = []): Promise<unknown> => {
...
test("Pi auth survives process recreation and concurrent locked updates", async (t) => {
  ...
  assert.deepEqual(
    await runPi(agentDir, `console.log(JSON.stringify(await AuthStorage.create().read('test')));`),
    { type: "api_key", key: "4" },
  );
...
test("Pi resumes separate sessions for stable worktree paths, including default-encoding collisions", async (t) => {
  ...
  for (const [index, cwd] of worktrees.entries()) {
    ...
    `,
        [cwd, join(agentDir, relative(AGENT_DIR, sessionDirectory(cwd)))],
      ),
      sessions[index],
    );
  }
