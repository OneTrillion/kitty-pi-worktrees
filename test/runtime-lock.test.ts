...
test("advisory lock persists after flock exits, rejects duplicates and aliases, and reuses its inode", { skip: process.platform !== "linux" }, async (t) => {
...
test("concurrent attempts have exactly one winner; different worktrees can both lock", { skip: process.platform !== "linux" }, async (t) => {
  const { base, root, worktree } = await setup(t);
  const results = await Promise.allSettled([acquireWorktreeLock(root, worktree), acquireWorktreeLock(root, worktree)]);
...
test("lock rejects symlinks, hard links and insecure files without overwriting data", { skip: process.platform !== "linux" }, async (t) => {
  const { base, root, worktree } = await setup(t);
...
test("SIGKILL releases the OS lock without stale-lock files needing repair", { skip: process.platform !== "linux", timeout: 10000 }, async (t) => {
  const { root, worktree } = await setup(t);
  const child = spawn(process.execPath, [fileURLToPath(new URL("./fixtures/lock-holder.ts", import.meta.url)), root, worktree], {
