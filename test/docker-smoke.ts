...
test("real Docker: persistence, stable mounts, UID/GID, socket boundary and isolated helper creation", {
  skip: !image && "Set PW_DOCKER_SMOKE_IMAGE to an already-built integrated image", timeout: 180000,
}, async () => {
  assert.ok(process.getuid!() > 0 && process.getgid!() > 0, "Run smoke tests as the non-root deployment user");
  const cleanups: Array<() => Promise<void>> = [];
  const f = await setupGit({ after: (fn) => { cleanups.push(fn); } });
  const volume = `pi-worktree-smoke-${randomUUID()}`;
  await writeFile(f.configPath, JSON.stringify({ ...f.config, image, agentVolume: volume,
  ...
  const server = await startRequestServer(config.runtimeRoot, async () => ({ version: 1, ok: true, op: "list", worktrees: [] }));
  const names: string[] = [];
  const prefix = ["--host", `unix://${config.dockerSocket}`, "--config", control.directory];
  const docker = async (args: string[]) => (await exec("/usr/bin/docker", [...prefix, ...args], {
    env: { PATH: "/usr/bin:/bin", HOME: control.directory }, cwd: control.directory,
    timeout: 120000, killSignal: "SIGKILL", maxBuffer: 1024 * 1024,
  })).stdout.trim();
  ...
  try {
    ...
    await withRepositoryLock(config, () => runHelper(config, client, {
      op: "create", branch: "smoke/task", destination, location: source,
      source: { branch: "main", head },
    }));
    const task = await locateGit(config, destination);
    const state = await withRepositoryLock(config, () => runHelper(config, client, { op: "inspect", location: task }));
    assert.equal(state?.upstream?.ref, "refs/heads/main");
    assert.equal(state?.dirty, false);

    for (const [location, reopen] of [[source, false], [task, false], [source, true]] as const) {
      const name = `pi-worktree-smoke-${randomUUID()}`;
      names.push(name);
      const args = dockerRunArgs(config, { ...location, socketPath: server.socketPath, uid: process.getuid!(), gid: process.getgid!() });
      ...
      assert.equal(await docker([...launch, "--pull=never", "--network=none", "--entrypoint", "node", config.image, "-e", script]), "smoke-ok");
    ...
    assert.equal(await docker(["run", "--rm", "--name", persistenceName, "--pull=never", "--network=none", "--user", `${process.getuid!()}:${process.getgid!()}`,
      "--mount", `type=volume,src=${volume},dst=/pi/agent`, "--entrypoint", "node", config.image,
      "-e", "console.log(require('fs').readFileSync('/pi/agent/smoke-persistence','utf8'))"]), "synthetic, not credentials");
  } finally {
