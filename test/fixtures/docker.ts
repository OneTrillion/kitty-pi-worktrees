...
import type { FakeContainer } from "./fake-docker-cli.ts";

export async function eventually<T>(read: () => Promise<T>, ready: (value: T) => boolean): Promise<T> {
  for (let i = 0; i < 500; i++) {
...
export async function setupDocker(t: TestContext) {
  const cleanups: Array<() => Promise<void>> = [];
  const fixture = await setupGit({ after(fn) { cleanups.push(fn); } });
  ...
  await new Promise<void>((resolve, reject) => { socketServer.once("error", reject); socketServer.listen(socketPath, resolve); });
  ...
  async function controls(changes: Record<string, unknown>) {
  ...
  const state = async (): Promise<FakeContainer[]> => JSON.parse(await readFile(statePath, "utf8")) as FakeContainer[];
  ...
  function run(mode: "start" | "recover" = "start", cwd = config.repositoryPath) {
    const abort = new AbortController();
    const result = runHostSession(config, cwd, mode, {
      dockerFactory: factory, containerUser: { uid: 1000, gid: 1000 }, signal: abort.signal,
      cleanupRetryMs: 10, onNotice: (message) => notices.push(message),
    });
  ...
  t.after(async () => {
    ...
    for (const item of await state()) {
      if (item.attachedPid) { try { process.kill(item.attachedPid, "SIGKILL"); } catch { /* Already exited. */ } }
    }
    await new Promise<void>((resolve) => socketServer.close(() => resolve()));
    for (const cleanup of cleanups) await cleanup();
  });
  return { ...fixture, config, engine, executable, factory, notices, state, save, controls, run,
    async calls(): Promise<Array<{ argv: string[]; env: Record<string, string>; pid: number }>> {
      try { return (await readFile(join(engine, "calls.jsonl"), "utf8")).trim().split("\n").filter(Boolean).map((line) => JSON.parse(line)); }
