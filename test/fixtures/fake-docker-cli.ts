...
import { FakeContainerSchema, type FakeContainer } from "./docker-state.ts";

const root = process.env.PW_FAKE_ENGINE!;
const statePath = join(root, "state.json");
const state = (): FakeContainer[] => {
  return FakeContainerSchema.array().parse(parseJson(readFileSync(statePath, "utf8")));
};
const save = (value: FakeContainer[]): void => {
  const temp = `${statePath}.${process.pid}`;
  writeFileSync(temp, JSON.stringify(value));
  renameSync(temp, statePath);
};
const controls = existsSync(join(root, "controls.json"))
  ? z.record(z.string(), z.unknown()).parse(parseJson(readFileSync(join(root, "controls.json"), "utf8")))
  : {};
const argv = process.argv.slice(2);
appendFileSync(
  join(root, "calls.jsonl"),
  JSON.stringify({ argv, env: process.env, pid: process.pid }) + "\n",
);
...
const fresh = (labels: Record<string, string>): FakeContainer => {
  return {
...
if (args[0] === "create") {
  ...
  if (controls.conflictOnCreate) {
    containers.push(fresh({}));
    save(containers);
    fail("raced with another container");
  }
  ...
  if (controls.mutateAfterCreate) {
    const { path, text } = z.object({ path: z.string(), text: z.string() }).parse(controls.mutateAfterCreate);
    writeFileSync(path, text);
  }
  if (controls.swapAfterCreate) {
    const { path, replacement, saved } = z
      .object({ path: z.string(), replacement: z.string(), saved: z.string() })
      .parse(controls.swapAfterCreate);
  ...
  const matches = state().filter((item) =>
    filter.startsWith("id=") ? item.Id === filter.slice(3) : item.Name === filter.slice("name=^".length, -1),
  );
