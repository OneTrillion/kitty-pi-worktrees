...
export const classifyState = (input: {
  ...
  upstream: { exists: boolean } | null;
  containedByUpstream: boolean;
  containsUpstream: boolean;
}): GitState["status"] => {
...
export const requireClean = (state: GitState): void => {
  if (state.conflicts || state.operation)
    throw new Error("Finish the active merge/rebase and resolve conflicts first");
