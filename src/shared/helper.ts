...
const path = z.string().min(1).max(8192).startsWith("/").refine((value) => !/[\p{Cc}\p{Cf}]/u.test(value));
...
export const HelperRequestSchema = z.discriminatedUnion("op", [
  ...
  z.strictObject({ op: z.literal("create"), location, destination: path, branch,
    source: z.strictObject({ branch: z.string().nullable(), head: z.string().regex(/^(?:[a-f0-9]{40}|[a-f0-9]{64})$/) }) }),
]);
