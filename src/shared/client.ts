...
export const requestSupervisor = async (
  ...
  return new Promise<Response>((resolve, reject) => {
    ...
    let settled = false;
    const finish = (result: Response | Error): void => {
      if (settled) return;
      settled = true;
      clearTimeout(timer);
    ...
    const timer = setTimeout(
      () => finish(new Error("Supervisor request timed out; check worktrees before retrying")),
      timeoutMs,
    );
    options.signal?.addEventListener("abort", abort, { once: true });
    socket.once("connect", () => socket.end(frame)); // Half-close: server dispatches only at EOF.
    socket.on("error", (cause) =>
      finish(new Error("Supervisor socket unavailable; start Pi through the host launcher", { cause })),
    );
    socket.on("data", (chunk: Buffer) => {
      try {
        decoder.push(chunk);
      } catch (cause) {
        finish(new Error("Invalid supervisor response frame", { cause }));
      }
    });
    socket.once("end", () => {
      if (settled) return;
      try {
        ...
        finish(new Error("Invalid supervisor response", { cause }));
      }
    });
    socket.once("close", () =>
      finish(new Error("Supervisor disconnected before a complete response; outcome may be unknown")),
    );
  });
};
