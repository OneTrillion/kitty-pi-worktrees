...
import { discoverCurrentWorktree, isAuthorizedWorktree, listGitWorktrees, localBranchExists, locateGit } from "./git-discovery.ts";
...
export function createWorktreeService(config: HostConfig, sourcePath: string, docker: DockerClient, options: WorktreeServiceOptions = {}) {
  const helper = options.helper ?? ((request, signal) => runHelper(config, docker, request, signal, options.user));
  const launch = options.launch ?? (async (path, branch, signal) => {
  ...
  async function inspect(record: WorktreeRecord, signal?: AbortSignal): Promise<Worktree> {
    const base = { ...record, id: worktreeId(record.path), upstream: null, open: false };
    try {
      if (!isAuthorizedWorktree(config, record.path)) throw new Error("Outside the configured host worktree paths");
      const location = await locateGit(config, record.path);
      base.open = await isWorktreeOpen(config.runtimeRoot, record.path);
      if (!base.open && await docker.lookup(containerName(record.path))) throw new Error("Leftover container; use the host recover command before opening/integrating");
      ...
      return { ...base, inspection: "unavailable", error: plainText(error instanceof Error ? error.message : "Inspection unavailable", 2000) };
    }
  }

  async function open(record: WorktreeRecord, signal?: AbortSignal): Promise<{ outcome: "reopened" | "already-active"; worktree: Worktree }> {
    ...
    signal?.throwIfAborted();
    try { await launch(record.path, record.branch, signal); }
    catch (error) { throw new RequestError("kitty-error", `${error instanceof Error ? error.message : "Kitty launch failed"}. Worktree preserved at ${record.path}; cd there and use the host start command`); }
    ...
    while (!await isWorktreeOpen(config.runtimeRoot, record.path)) {
      signal?.throwIfAborted();
      if (Date.now() >= deadline) throw new RequestError("unavailable", "Kitty accepted the tab but startup was not confirmed. Check the new tab before retrying; the worktree is preserved");
  ...
  async function dispatch(request: Request, signal?: AbortSignal): Promise<Response> {
    ...
    if (request.op === "list") {
      ...
      for (const record of records) { signal?.throwIfAborted(); worktrees.push(await inspect(record, signal)); }
    ...
    if (request.op === "open" || request.op === "inspect") {
      ...
      if (request.op === "inspect") return { version: 1, ok: true, op: "inspect", worktree: await inspect(record, signal) };
    ...
    if (matches.length > 1) throw new RequestError("git-error", "Branch is checked out in multiple worktrees; resolve this Git state explicitly");
    if (matches[0]) return { version: 1, ok: true, op: "create-or-open", ...await open(matches[0], signal) };
    const source = await discoverCurrentWorktree(config, sourcePath, signal);
    if (!source.branch && !await localBranchExists(config, request.branch, signal)) throw new RequestError("git-error", "Cannot create a task from detached HEAD; check out a local source branch first");
    const path = deriveWorktreePath(config.worktreeRoot, request.branch);
    try { await mkdir(path); }
    catch (cause) {
      if ((cause as NodeJS.ErrnoException).code === "EEXIST") throw new RequestError("path-collision", `Destination already exists: ${path}. Nothing was overwritten`);
      throw cause;
    }
    ...
    try {
      await helper({ op: "create", branch: request.branch, destination: path,
        location: { worktreePath: config.repositoryPath, gitDir: join(config.repositoryPath, ".git"), commonGitDir: join(config.repositoryPath, ".git") },
        source: { branch: source.branch, head: source.head } }, signal);
    } catch (error) {
      throw new RequestError("git-error", `${error instanceof Error ? error.message : "Git creation failed"}. Inspect preserved branch/worktree state and destination ${path} before retrying`);
    }
    const created = (await listGitWorktrees(config, signal)).find((item) => item.path === path && item.branch === request.branch);
    if (!created) throw new RequestError("git-error", "Git did not report the new worktree; inspect its preserved state on the host");
  ...
  return {
    async handle(input: Request, signal?: AbortSignal): Promise<Response> {
      try {
        ...
        return { version: 1, ok: false, error: { code: error instanceof RequestError ? error.code : "git-error",
          message: plainText(error instanceof Error ? error.message : "Host operation failed", 4000) || "Host operation failed" } };
      }
    },
  };
}
