...
export const kittyLaunchArgs = (
  ...
  if (
    ![socket, configPath, worktreePath].every((path) => isAbsolute(path) && !/[\p{Cc}\p{Cf}]/u.test(path))
  ) {
...
export const launchInKitty = async (
  config: HostConfig,
  configPath: string,
  path: string,
  branch: string | null,
  ...
  if (!selected)
    throw new Error("Configure kittySocket or launch from Kitty with socket-only remote control enabled");
  ...
  if (
    ...
    [config.repositoryPath, config.worktreeRoot].some(
      (root) => pathsOverlap(root, socket) || pathsOverlap(root, binary),
    )
  ) {
  ...
  try {
    await exec(binary, kittyLaunchArgs(socket, configPath, path, branch), {
      ...
      env: { PATH: "/usr/bin:/bin", HOME: "/nonexistent", LANG: "C", TERM: "xterm-256color" },
      timeout: 10000,
      killSignal: "SIGKILL",
      maxBuffer: 8192,
      ...(signal ? { signal } : {}),
    ...
    throw new Error("Kitty launch failed; the worktree is preserved. Open it with the host start command", {
      cause,
    });
  }
};
