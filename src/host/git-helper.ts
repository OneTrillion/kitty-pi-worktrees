...
import { getHostUser } from "./runtime.ts";

export const gitHelperName = (config: HostConfig): string => {
...
export const withRepositoryLock = async <T>(
  ...
  for (;;) {
    ...
    try {
      lock = await acquireWorktreeLock(config.runtimeRoot, join(config.repositoryPath, ".git"));
    } catch (error) {
...
export const gitHelperArgs = (
  ...
  if (
    ...
    [user.uid, user.gid].some((id) => !Number.isSafeInteger(id) || id < 1 || id > 0xfffffffe)
  ) {
  ...
  if (
    !isAuthorizedWorktree(config, request.location.worktreePath) ||
    (request.op === "create" &&
      (dirname(path) !== config.worktreeRoot ||
        request.location.worktreePath !== config.repositoryPath ||
        request.location.gitDir !== common))
  ) {
  ...
  return [
    ...
    "--label",
    `${REPOSITORY_LABEL}=${worktreeId(common)}`,
    "--label",
    `${RUN_LABEL}=${runId}`,
    "--tmpfs",
    ...
    "HOME=/tmp",
    "--mount",
    `type=bind,src=${path},dst=${path}${readonly}`,
    "--mount",
    `type=bind,src=${common},dst=${common}${readonly}`,
    "--entrypoint",
    "node",
    config.image,
    "/opt/pi-worktree/dist/git/worker-cli.js",
...
export const runHelper = async (
  ...
  if (await docker.lookup(name))
    throw new Error("Git helper still exists; use host recover-git before retrying");
  ...
  const revalidateDirectories = await pinMountDirectories([
    ...
    config.worktreeRoot,
    request.location.worktreePath,
    request.location.gitDir,
    request.location.commonGitDir,
  ...
  const revalidate = async (): Promise<void> => {
    ...
    if (
      current.gitDir !== request.location.gitDir ||
      current.commonGitDir !== request.location.commonGitDir
    ) {
  ...
  try {
    ...
    if (!info || info.Name !== `/${name}` || !owned(info, scope, runId, "git"))
      throw new Error("Git helper identity was not verified");
    id = info.Id;
    await revalidate();
    const reply = HelperReplySchema.parse(parseJson(await docker.capture(id, signal)));
...
export const recoverGitHelper = async (config: HostConfig, docker: DockerClient): Promise<void> => {
  ...
  try {
    ...
    if (!info) return;
    const scope = { worktreePath: config.repositoryPath, commonGitDir: join(config.repositoryPath, ".git") };
    if (!owned(info, scope, undefined, "git"))
      throw new Error("Unrecognized container occupies the Git helper name");
    await cleanupContainer(docker, scope, info.Config.Labels[RUN_LABEL], info.Id, 2000, console.error, {
      name,
      role: "git",
    });
  } finally {
