...
export const createDockerClient = async (
  config: HostConfig,
  ...
  if (
    !socket.isSocket() ||
    (socket.uid !== 0 && socket.uid !== process.getuid?.()) ||
  ...
  const command = async (args: string[], signal?: AbortSignal, timeout = 20000): Promise<string> => {
    try {
      const { stdout } = await exec(binary, [...prefix, ...args], {
        cwd: privateConfigDir,
        env,
        encoding: "utf8",
        timeout,
        killSignal: "SIGKILL",
  ...
  const matchingIds = async (target: string): Promise<string[]> => {
  ...
  return {
    async lookup(target) {
      if (!/^(?:[a-f0-9]{64}|pi-worktree-(?:git-)?[a-f0-9]{64})$/.test(target))
      ...
      try {
        const info = ContainerSchema.parse(
          parseJson(await command(["container", "inspect", "--format", "{{json .}}", id])),
        );
    ...
    async stop(id) {
      await command(["container", "stop", "--time", "10", idSchema.parse(id)]);
    },
    async remove(id) {
      await command(["container", "rm", idSchema.parse(id)]);
    },
  };
};
